...
static void USB_RecvTask(void *param)
{
    while (1)
    {
        ...
        do{
            ...
            {
                USBD_CDC_SetRxBuffer(&hUsbDeviceHS, UserRxBufferHS+buffer_index-current_cdc_pack_size);
                buffer_index=-1;
                if(Recv_Overflow_cb)
                    Recv_Overflow_cb(kUserData);
            }
            else
                USBD_CDC_SetRxBuffer(&hUsbDeviceHS, UserRxBufferHS+buffer_index);     //防止缓冲区溢出，重设接收缓冲区地址 
        }while(current_cdc_pack_size==64);
        if(buffer_index>0)
           Recv_Finished_Cb(UserRxBufferHS, buffer_index);
    }
}
