...
#include "pid.h"
#define cheat FALSE  //作弊模式 去掉较小的gyro值	TRUE FALSE
...
void INS_Task(BMI088_data *data)  //1khz
{
    ...
    if ((count % 1) == 0)
    {
        BMI088_read(gyro, accel, &temp); // 读取陀螺仪(角速度)和加速度计(线性加速度)
				if(first_mahony==0)
				{
					first_mahony++;
				...
				if(attitude_flag==2)  //ekf的姿态解算
				{
					...
					yaw=Get_Yaw();//获得yaw
					Get_Quaternion(data->q); //获得四元数
					Get_AngularVelocity(data->Gyro); //获得角速度
					Get_AngularAcceleration(data->Accel);//获得加速度
					//==============================================================================
				}
